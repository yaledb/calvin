...
TxnProto* TPCC::NewTxn(int64 txn_id, int txn_type, string args,
  ...
  int remote_node;
  txn->set_txn_node(config->this_node_id);
  ...
  if (mp) {
    ...
    txn->add_txn_other_node(remote_node);
  }
     ...
     case ORDER_STATUS :
     {
      ...
      case STOCK_LEVEL:
      {
        ...
        for(int i = latest_order_number; (i >= 0) && (i > latest_order_number - 20); i--) {
          snprintf(order_key, sizeof(order_key), "%so%d", district_key, i);
          ...
          for(int j = 0; j < ol_number;j++) {
            ...
            items_used.insert(item);
            txn->add_read_set(order_line_key);
            snprintf(stock_key, sizeof(stock_key), "%ssi%d", warehouse_key, item);
...
int TPCC::PaymentTransaction(TxnProto* txn, StorageManager* storage) const {
  ...
  snprintf(history_data, sizeof(history_data), "%s    %s", warehouse->name().c_str(), district->name().c_str());
