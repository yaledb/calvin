...
TxnProto* Microbenchmark::MicroTxnMP(int64 txn_id, int part1, int part2, int part3, int part4) {
  assert(part1 != part2 && part3 != part1 && part3 != part2);
  // Create the new transaction object
  TxnProto* txn = new TxnProto();
  ...
  txn->set_txn_node(part1);
  txn->add_txn_other_node(part2);
  txn->add_txn_other_node(part3);
  txn->add_txn_other_node(part4);
  ...
  txn->add_read_write_set(IntToString(hotkey4));
