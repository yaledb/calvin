...
unordered_map<int, std::vector<int> > index_;
vector<pthread_mutex_t> mutex_for_index;
...
class MClient : public Client {
 public:
  ...
  virtual ~MClient() {}
  virtual void GetTxn(TxnProto** txn, int txn_id, int dependent) {
    if (config_->all_nodes.size() > 1 && rand() % 100 < percent_mp_) {
      ...
      if (dependent == 1) {
        int index = rand() % INDEX_NUMBER;
        *txn = microbenchmark.MicroTxnDependentMP(txn_id, config_->this_node_id, other, index);
      } else {
      ...
      if (dependent == 1) {
        int index = rand() % INDEX_NUMBER;
        *txn = microbenchmark.MicroTxnDependentSP(txn_id, index);
      } else {
...
//---------------------------Creat index------------------------------------
int key;
int nparts = config.all_nodes.size();
int key_start = config.all_nodes.size()*HOT;
int part = config.all_nodes.size() - config.this_node_id - 1;

for(int i = 0; i < INDEX_NUMBER; i++) {
  set<int> myset;
  vector<int> myvector;
    
  int hotkey = part + nparts * i;
  myvector.push_back(hotkey);
  
  for(int j = 0; j < 9; j++) {
    do {
      key = key_start + part + nparts * (rand() % ((1000000 - key_start)/nparts));
    } while(myset.count(key));
    myset.insert(key);
...
//-------------------------------------------------------------------------
mutex_for_index.resize(INDEX_NUMBER);
for (int i = 0; i < INDEX_NUMBER; i++) {
  pthread_mutex_init(&mutex_for_index[i], NULL);
}
...
storage->Initmutex();
  ...
  Sequencer sequencer(&config, &multiplexer, client, storage);
