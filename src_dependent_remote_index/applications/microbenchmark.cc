...
TxnProto* Microbenchmark::MicroTxnDependentSP(int64 txn_id, int64 index_number) {
  TxnProto* txn = new TxnProto();
  txn->set_txn_id(txn_id);
  txn->set_index_number(index_number);
  txn->set_txn_type(MICROTXN_DEPENDENT_SP);
  return txn;
}

// Create a dependent multi-partition transaction
TxnProto* Microbenchmark::MicroTxnDependentMP(int64 txn_id, int part1, int part2, int64 index_number) {
  TxnProto* txn = new TxnProto();
  txn->set_txn_id(txn_id);
  txn->set_index_number(index_number);
  txn->set_index_node(part1);
  txn->set_other_node(part2);
  txn->set_txn_type(MICROTXN_DEPENDENT_MP);
  ...
  for (int i = 0; i < kRWSetSize; i++) {
    ...
    *val = IntToString(StringToInt(*val) + 1);
    ...
    //   storage->PutObject(txn->read_write_set(i), val);
        // Uncomment the following code if tested "short" transaction
        int x = 1;
