...
TxnProto* TPCC::NewTxn(int64 txn_id, int txn_type, string args,
                       Configuration* config, int partition_id) const {
  ...
  int remote_partition;
  if (mp) {
    do {
      remote_partition = rand() % config->partitions_size;
    } while (config->partitions_size > 1 &&
             remote_partition == partition_id);
  ...
  switch (txn_type) {
    ...
    case NEW_ORDER:
      // First, we pick a local warehouse
        ...
        warehouse_id = (rand() % WAREHOUSES_PER_NODE) * config->partitions_size+ partition_id;
...
if(next_order_id_for_district.count(district_key)>0) {
  ...
  next_order_id_for_district[district_key] = 1;
      ...
      for (int i = 0; i < order_line_count; i++) {
        ...
        if (mp) {
          ...
          remote_warehouse_id = (rand() % WAREHOUSES_PER_NODE) *
                                            config->partitions_size + remote_partition;
          snprintf(remote_warehouse_key, sizeof(remote_warehouse_key),
      ...
      tpcc_args->set_major_partition(partition_id);
    ...
    case PAYMENT:
      ...
      if (WAREHOUSES_PER_NODE * config->partitions_size == 1 || !mp) {
        customer_id = rand() % CUSTOMERS_PER_DISTRICT;
        snprintf(customer_key, sizeof(customer_key),
                 "w%dd%dc%d",
                 warehouse_id, district_id, customer_id);
...
snprintf(customer_key, sizeof(customer_key), "%s",
        customer_string.c_str());
snprintf(warehouse_key, sizeof(warehouse_key), "%s",
        warehouse_string.c_str());
snprintf(district_key, sizeof(district_key), "%s",
        district_string.c_str());
...
customer_order_line_number = order_line_number[customer_latest_order];
      ...
      snprintf(order_key, sizeof(order_key), "%s",
               customer_latest_order.c_str());
         ...
         for(int i = 0; i < DISTRICTS_PER_WAREHOUSE; i++) {
           ...
           if((smallest_order_id_for_district.count(district_key) == 0) || (smallest_order_id_for_district[district_key] > latest_order_id_for_district[district_key])) {
            continue;
             ...
             snprintf(new_order_key, sizeof(new_order_key),
                      "%sno%d", district_key, oldest_order);
             txn->add_read_write_set(new_order_key);
           }

           snprintf(order_key, sizeof(order_key),
                   "%so%d", district_key, oldest_order);
...
string customer_string = customer_for_order[order_key];
           snprintf(customer_key, sizeof(customer_key),
                      "%s", customer_string.c_str());
...
int TPCC::NewOrderTransaction(TxnProto* txn, StorageManager* storage) const {
    ...
    ValueStore* warehouse_value = storage->ReadObject(txn->read_set(0));
    assert(warehouse->ParseFromString(warehouse_value->value));
    warehouse_value->share_lock_number--;

    // Next, we retrieve the district
    District* district = new District();
    ValueStore* district_value = storage->ReadObject(txn->read_write_set(0));
    assert(district->ParseFromString(district_value->value));

    // Increment the district's next order ID and put to datastore
    district->set_next_order_id(district->next_order_id() + 1);
    assert(district->SerializeToString(&(district_value->value)));
    district_value->exclusive_lock_number--;

    // Retrieve the customer we are looking for
    Customer* customer = new Customer();
    ValueStore* customer_value = storage->ReadObject(txn->read_set(1));
    assert(customer->ParseFromString(customer_value->value));
    customer_value->share_lock_number--;
    ...
    int major_partition = tpcc_args->major_partition();
    ...
    for (int i = 0; i < order_line_count; i++) {
      ...
      snprintf(order_line_key, sizeof(order_line_key), "%s",
              txn->write_set(i).c_str());
      ...
      ValueStore* stock_value = storage->ReadObject(stock_key);
      assert(stock->ParseFromString(stock_value->value));
      stock_value->exclusive_lock_number--;
      ...
      assert(stock->SerializeToString(&(stock_value->value)));
      ...
      ValueStore* order_line_value_store = new ValueStore((*order_line_value), 0, 0);
      storage->PutObject(order_line_key, order_line_value_store);
      //order->add_order_line_ptr(reinterpret_cast<uint64>(order_line_value));
pthread_mutex_lock(&mutex_for_item);
if (storage->partition_id_ == major_partition)
...
pthread_mutex_unlock(&mutex_for_item);
    ...
    char new_order_key[128];
    snprintf(new_order_key, sizeof(new_order_key), "%s",
              txn->write_set(order_line_count).c_str());
    ...
    ValueStore* new_order_value_store = new ValueStore((*new_order_value), 0, 0);
    storage->PutObject(new_order_key, new_order_value_store);

    char order_key[128];
    snprintf(order_key, sizeof(order_key), "%s",
             txn->write_set(order_line_count + 1).c_str());
    ...
    ValueStore* order_value_store = new ValueStore((*order_value), 0, 0);
    storage->PutObject(order_key, order_value_store);
...
int TPCC::PaymentTransaction(TxnProto* txn, StorageManager* storage) const {
    ...
    if (tpcc_args->has_last_name()) {
      ...
      if (txn->read_write_set_size() < 3 ||
          secondary_key != txn->read_write_set(2)) {
    ...
    ValueStore* warehouse_value = storage->ReadObject(warehouse_key);
    Warehouse* warehouse = new Warehouse();
    assert(warehouse->ParseFromString(warehouse_value->value));
    // Next, we update the values of the warehouse and write it out
    warehouse->set_year_to_date(warehouse->year_to_date() + amount);
    assert(warehouse->SerializeToString(&(warehouse_value->value)));
    warehouse_value->exclusive_lock_number--;
    ...
    ValueStore* district_value = storage->ReadObject(district_key);
    District* district = new District();
    assert(district->ParseFromString(district_value->value));
    ...
    customer->set_year_to_date_payment(customer->year_to_date_payment() +
                                       amount);
    customer->set_payment_count(customer->payment_count() + 1);
    customer_value->exclusive_lock_number--;
    ...
    assert(customer->SerializeToString(&(customer_value->value)));
    ...
    Value* history_value_data = new Value();
    ValueStore* history_value = new ValueStore((*history_value_data), 0, 0);
    assert(history->SerializeToString(history_value_data));
...
int TPCC::OrderStatusTransaction(TxnProto* txn, StorageManager* storage) const {
  ...
  warehouse_value->share_lock_number--;
  District* district = new District();
  ValueStore* district_value = storage->ReadObject(txn->read_set(1));
  assert(district->ParseFromString(district_value->value));
  district_value->share_lock_number--;
  ValueStore* customer_value;
  Customer* customer = new Customer();
  ...
  customer_value->share_lock_number--;
  Order* order = new Order();
  ValueStore* order_value = storage->ReadObject(txn->read_set(3));
  assert(order->ParseFromString(order_value->value));
  order_value->share_lock_number--;

  for(int i = 0; i < order_line_count; i++) {
    OrderLine* order_line = new OrderLine();
    ValueStore* order_line_value = storage->ReadObject(txn->read_set(4+i));
    assert(order_line->ParseFromString(order_line_value->value));
...
int TPCC::StockLevelTransaction(TxnProto* txn, StorageManager* storage) const { 
  ...
  for(int i = 0; i < cycle; i++) {
      ...
      OrderLine* order_line = new OrderLine();
      ValueStore* order_line_value = storage->ReadObject(txn->read_set(2+index));
      index ++;
      assert(order_line->ParseFromString(order_line_value->value));
      string item_key = order_line->item_id();
      order_line_value->share_lock_number--;
      ...
      Stock* stock = new Stock();
      ValueStore* stock_value = storage->ReadObject(txn->read_set(2+index));
      index ++;
      assert(stock->ParseFromString(stock_value->value));
      if(stock->quantity() < threshold) {
        low_stock ++;
      }
      stock_value->share_lock_number--;      
...
string district_string = txn->read_set(i);
    District* district = new District();
    ValueStore* district_value = storage->ReadObject(district_string);
    assert(district->ParseFromString(district_value->value));
    district_value->share_lock_number--;
    ...
    Order* order = new Order();
    ValueStore* order_value = storage->ReadObject(txn->read_write_set(read_write_index));
    read_write_index ++;
    assert(order->ParseFromString(order_value->value));
    ...
    order_value->exclusive_lock_number--;
    ...
    for(int j = 0; j < ol_number; j++) {
      OrderLine* order_line = new OrderLine();
      ValueStore* order_line_value = storage->ReadObject(txn->read_write_set(read_write_index));
      read_write_index ++;
      assert(order_line->ParseFromString(order_line_value->value));
      order_line->set_delivery_date(GetTime());
      total_amount = total_amount + order_line->amount();
      order_line_value->exclusive_lock_number--;
    ...
    Customer* customer = new Customer();
    ValueStore* customer_value = storage->ReadObject(txn->read_write_set(read_write_index));
    read_write_index ++;
    assert(customer->ParseFromString(customer_value->value));
...
if (conf->LookupPartition(warehouse_key) != partition_id) {
  continue;
}
    ...
    if (conf->LookupPartition(warehouse_key) == partition_id) {
      storage->PutObject(warehouse_key, new ValueStore(warehouse_value, 0, 0));
      storage->PutObject(warehouse_key_ytd, new ValueStore(warehouse_value, 0, 0));
    }
    ...
    for (int j = 0; j < DISTRICTS_PER_WAREHOUSE; j++) {
      ...
      // Finally, we pass it off to the storage manager to write to disk
      if (conf->LookupPartition(district_key) == partition_id) {
        storage->PutObject(district_key, new ValueStore(district_value, 0, 0));
        storage->PutObject(district_key_ytd, new ValueStore(district_value, 0, 0));
      }
      ...
      for (int k = 0; k < CUSTOMERS_PER_DISTRICT; k++) {
        ...
        // Finally, we pass it off to the storage manager to write to disk
        if (conf->LookupPartition(customer_key) == partition_id)
          storage->PutObject(customer_key, new ValueStore(customer_value, 0, 0));
          delete customer;
      }
    ...
    for (int j = 0; j < NUMBER_OF_ITEMS; j++) {
      ...
      // Finally, we pass it off to the storage manager to write to disk
      if (conf->LookupPartition(stock->id()) == partition_id)
        storage->PutObject(stock->id(), new ValueStore(stock_value, 0, 0));
        delete stock;
    }
    // Free storage
    delete warehouse;
