...
class StorageManager {
 public:
  ...
  StorageManager(Configuration* config, Connection* connection, Storage* actual_storage,
                 TxnProto* txn, int partition_id,ValueStore* cache_storage[]);

  ~StorageManager();

  ValueStore* ReadObject(const Key& key);
  bool PutObject(const Key& key, ValueStore* value);
...
// private:
  ...
  vector<ValueStore*> remote_reads_;
