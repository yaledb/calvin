...
int part = scheduler->configuration_->this_node_id;
...
void* DeterministicScheduler::RunWorkerThread(void* arg) {
  ...
  while (true) {
    ...
    if (got_message == true) {
      ...
      scheduler->index_connection->Send(message);
    } else {
     ...
     bool got_it = scheduler->txns_queue->Pop(&txn);
      if (got_it == true) {
        ...
        if (txn->txn_type() == 3) {
          ...
          int index_number = txn->index_number();
          vector<int> aa = index_[index_number];
          for (int i = 0; i < 10; i++) {
            if (aa[i] != StringToInt(txn->read_write_set(i))) {
              restart = 1;
              break;
            }
          }
