        ...
        if(got_message == true) {
          if (message.type() == MessageProto::REMOTE_INDEX_RESULT) {
            ...
            if (txn->txn_type() == 3) {
              ...
              } else if (txn->read_write_set_size() == 10) {
                for (int i = 0; i < message.keys_size(); i++) {
                  txn->set_read_write_set(5 + j, message.keys(i));
                  j++;
                  if (j >= 5) {
                    break;
                  }
          ...
          if (rand() % 100 < DEPENDENT_PERCENT) {
            got_message = restart_queues->Pop(&message);
            if (got_message == true) {
              ...
              message.set_destination_node(configuration_->all_nodes.size() - configuration_->this_node_id - 1);
