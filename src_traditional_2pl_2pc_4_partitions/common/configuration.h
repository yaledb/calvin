...
// The work thread number of one node
#define WorkersNumber  5
#define SPINTIME 0.000001
...
struct Node {
  ...
  int node_id;
  int replica_id;
  // Globally unique partition identifier.
  int partition_id;
...
class Configuration {
 public:
  ...
  // Tracks the set of current active partitions in the system.
//  map<int, Partition*> all_partitions;
  // partition number of this node
//  vector<int> partitions_of_this_node;
