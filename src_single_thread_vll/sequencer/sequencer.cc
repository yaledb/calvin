...
#ifdef LATENCY_TEST1
   if(txn->txn_id()%SAMPLE_RATE == 0) {
     execute_begin[txn->txn_id()/SAMPLE_RATE] = GetTime();
   }
#endif
...
#endif
std::cout << "Starting sequencer, enter RunReader.\n" << std::flush;
  // Set up batch messages for each system node.
  map<int, MessageProto> batches;
  for (int i = 0; i < (int)configuration_->partitions_size; i++) {
	string channel("scheduler");
	channel.append(IntToString(i%WorkersNumber));
    batches[i].set_destination_channel(channel);
    batches[i].set_destination_node(i/WorkersNumber);
